...
serve(async (req) => {
  ...
  try {
    ...
    // Simple, direct prompt that focuses on preserving the original image content
    const enhancementPrompt = `Enhance this exact image. Keep everything the same - same subject, same composition, same scene. Just improve the quality: make it sharper, clearer, reduce noise, enhance colors slightly. Do NOT change or replace anything in the image. Output the same image but enhanced.`;
